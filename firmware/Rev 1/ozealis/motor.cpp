...
static inline uint8_t phaseU(uint16_t ang) {
  return ang & 0xFF;
}  //   0°
static inline uint8_t phaseV(uint16_t ang) {
  return (ang + 85) & 0xFF;
}  // +120°
static inline uint8_t phaseW(uint16_t ang) {
  return (ang + 171) & 0xFF;
}  // +240°
...
void setupMotor() {
  ...
  ledcAttach(PHASE_U, CH_U, LEDC_RESOLUTION);
  ledcAttach(PHASE_V, CH_V, LEDC_RESOLUTION);
  ledcAttach(PHASE_W, CH_W, LEDC_RESOLUTION);
